...
func Auth(r *ghttp.Request) {
	...
	// 验证 Token 是否有效
	if isValidToken := logic.JwtHandler.Valid(r.Context(), token); !isValidToken {
		handleError(r, 401, err, "Token 验证失败")
	}
...
func parseTokenAndSetContext(r *ghttp.Request, token string) error {
	claims, err := logic.JwtHandler.Parse(r.Context(), token)
	if err != nil {
		return err
	}
	username := claims.Username
	logic.CtxHandler.SetUserContext(username, r.SetCtxVar)
	return nil
}
